...
public class Q26cGenderOfChronicallyHomelessPersonDataBeanMaker extends BaseBeanMaker {
	...
	public static List<Q26cGenderOfChronicallyHomelessPersonDataBean> getQ26cGenderOfChronicallyHomelessPersonList(ReportData data){
		...
		try {
			...
			String transFemaleCHQuery ="select distinct(e.dedup_client_id) from %s.enrollment e,%s.client c,%s.project p  where c.id =e.client_id and  and entrydate <= :endDate c.gender = '2' and e.disablingcondition in ('8','9') and e.projectid = p.id %p  and e.id not in ( select enrollmentid from %s.exit ext where  ext.exitdate is null  or  ext.exitdate <= :startDate ) ";
			String transMaleCHQuery ="select distinct(e.dedup_client_id) from %s.enrollment e,%s.client c,%s.project p  where c.id =e.client_id and entrydate <= :endDate  c.gender = '3' and e.disablingcondition ='99' and e.projectid = p.id %p  and e.id not in ( select enrollmentid from %s.exit ext where  ext.exitdate is null  or  ext.exitdate <= :startDate ) ";
			String genderNotConfirmingCHQuery ="select distinct(e.dedup_client_id) from %s.enrollment e,%s.client c,%s.project p  where c.id =e.client_id and  and entrydate <= :endDate c.gender = '4' and e.disablingcondition ='99' and e.projectid = p.id %p  and e.id not in ( select enrollmentid from %s.exit ext where  ext.exitdate is null  or  ext.exitdate <= :startDate ) ";
