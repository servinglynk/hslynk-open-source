...
public class ClientCashIncomeCategoryBeanMaker extends BaseBeanMaker {
	
	public static List<Q18ClientCashIncomeCategoryEarnedOtherIncomeDataBean> getClientCashIncomeCategoryEarnedOtherIncomeList(ReportData data,boolean youthLogic){
		...
		if(youthLogic) {
			List<String> youthList = data.getYouthList();
			incomeAtEntry = incomeAtEntry.parallelStream().filter(income -> youthList.contains(income.getDedupClientId()) ).collect(Collectors.toList());
			incomeAtExit = incomeAtExit.parallelStream().filter(income -> youthList.contains(income.getDedupClientId()) ).collect(Collectors.toList());
			incomeAndSourcesAtAnnualAssesment = incomeAndSourcesAtAnnualAssesment.parallelStream().filter(income -> youthList.contains(income.getDedupClientId()) ).collect(Collectors.toList());
		}
