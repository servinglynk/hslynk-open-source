...
public class Q25ePhysicalAndMentalHealthConditionsVeteransDataBeanMaker extends BaseBeanMaker {
	...
	public static List<Q25ePhysicalAndMentalHealthConditionsVeteransDataBean> getQ25ePhysicalAndMentalHealthConditionsVeteransList(ReportData data){
		...
		try {
			if(data.isLiveMode()) {
				String query = " select distinct(dedup_client_id) from %s.disabilities d,%s.enrollment e  where e.id = d.enrollmentid and d.datacollectionstage = '1' and d.information_date >= :startDate and d.information_date <= :endDate ";
				String queryExit = " select distinct(dedup_client_id) from %s.disabilities d, %s.enrollment e, %s.exit ext  where e.id = d.enrollmentid and e.id=ext.enrollmentid and d.datacollectionstage = '3' and d.information_date >= :startDate and d.information_date <= :endDate ";
				String queryStayers = " select distinct(dedup_client_id) from %s.disabilities d, %s.enrollment e where e.id = d.enrollmentid and d.datacollectionstage = '4' and d.information_date >= :startDate and d.information_date <= :endDate ";
