...
public static List<Q26bNumberOfChronicallyHomelessPersonsByHouseholdDataBean> getQ26bNumberOfChronicallyHomelessPersonsByHouseholdList(ReportData data){
	...
	try {
		if(data.isLiveMode()) {
			...
			String chronicHomelessQuery ="select distinct(dedup_client_id) from %s.enrollment e,%s.client c,%.project p  where c.id =e.client_id and e.chronichomeless='true' and e.projectid = p.id %p";
			String noChronicHomelessQuery ="select distinct(dedup_client_id) from enrollment e,%s.client c,%.project p  where c.id =e.client_id  and e.chronichomeless='false' and e.projectid = p.id %p";
			String dnKChHomelessQuery ="select distinct(dedup_client_id) from enrollment e,%s.client c,%.project p  where c.id =e.client_id and  and e.disablingcondition in ('8','9') and e.projectid = p.id %p";
			String dnCChHomelessQuery ="select distinct(dedup_client_id) from enrollment e,%s.client c,%.project p  where c.id =e.client_id and e.disablingcondition ='99' and e.projectid = p.id %p";
