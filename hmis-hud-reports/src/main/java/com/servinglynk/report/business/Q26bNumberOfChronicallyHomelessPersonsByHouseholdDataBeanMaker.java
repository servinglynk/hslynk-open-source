...
public static List<Q26bNumberOfChronicallyHomelessPersonsByHouseholdDataBean> getQ26bNumberOfChronicallyHomelessPersonsByHouseholdList(ReportData data){
	...
	try {
		if(data.isLiveMode()) {
			...
			String dnKChHomelessQuery ="select distinct(e.dedup_client_id) from enrollment e,%s.client c,%s.project p  where c.id =e.client_id and e.disablingcondition in ('8','9') and e.projectid = p.id %p ";
