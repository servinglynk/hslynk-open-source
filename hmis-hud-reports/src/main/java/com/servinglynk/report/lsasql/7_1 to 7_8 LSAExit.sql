CREATE OR REPLACE FUNCTION lsa.run_lsaexit (IN reportId integer, IN project_group_code varchar)
RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN

WITH subquery AS (select distinct cd.Cohort, hhid.EnrollmentID
	...
	left outer join lsa.tlsa_HHID disqualify on disqualify.HouseholdID = hhid.HouseholdID
		...
		and (select coc_code
			 ...
			 where CAST (enrollmentid AS varchar) = disqualify.EnrollmentID
			    and deleted is false and parent_id is null
...
update lsa.tlsa_HHID hhid
	set ExitCohort = subquery.Cohort
		, Step = '7.1'
   from subquery
  where subquery.EnrollmentID = hhid.EnrollmentID;
...
*/
	...
	insert into lsa.tlsa_Exit (Cohort, hohid, HHType,
--	QualifyingExitHHID, ExitFrom, ExitTo,
	ReportID, Step)
...
WITH subquery AS (select hhid.ExitCohort,
       hhid.hohid,
	   (case hhid.ExitCohort when -2 then hhid.Exit2HHType
			when -1 then hhid.Exit1HHType
			else hhid.ActiveHHType end) as HHType,
	  (select qx.HouseholdID
		from lsa.tlsa_HHID qx
	   where qx.ExitCohort = ex.Cohort and qx.HoHID = ex.HoHID
		...
		order by case when qx.ExitDest between 1 and 6 then 2
					 ...
					 , qx.ExitDest asc
					 , qx.EntryDate desc
					 , qx.EnrollmentID desc limit 1) as QualifyingExitHHID,
	    (case
              when hhid.ProjectType = 1 then 2
              when hhid.ProjectType = 2 then 3
              when hhid.ProjectType = 8 then 4
              ...
              when hhid.ProjectType = 3 and hhid.MoveInDate is not null then 6
              when hhid.ProjectType = 13 then 7
              else 8 end) as ExitFrom
		, hhid.ExitDest as ExitTo
  from lsa.tlsa_HHID hhid
 inner join lsa.lsa_Report rpt on rpt.ReportEnd >= hhid.EntryDate
 inner join lsa.tlsa_exit ex   on (hhid.ExitCohort = ex.Cohort and hhid.HoHID = ex.HoHID
		 and (case ex.Cohort when -2 then hhid.Exit2HHType
				when -1 then hhid.Exit1HHType
				else hhid.ActiveHHType end
				= ex.HHType))
 where hhid.ExitCohort is not null)
 update lsa.tlsa_exit ex
	set QualifyingExitHHID = subquery.QualifyingExitHHID
	    ,ExitFrom = subquery.ExitFrom
		,ExitTo   = subquery.ExitTo
		,Step = '7.2.1'
   from subquery
  where ex.Cohort =  subquery.ExitCohort
    and ex.hohid  =  subquery.hohid
	and ex.HHType =  subquery.HHType;
...
*/
    WITH subquery AS (
	  select  ex.hohid as shohid
	        , ex.cohort as scohort
			, ex.QualifyingExitHHID as sQualifyingExitHHID
            , (select max(case when c.veteran_status = '1' then 1 else 0 end)
				from v2020.client c
				where cast (c.id as varchar) = hh.PersonalID and c.deleted is false and c.parent_id is null) as sHHVet
			, (select max(case when disability.DisabilityStatus = 1 then 1 else 0 end)
				from lsa.tlsa_Enrollment disability
				where disability.HouseholdID = hh.HouseholdID) as sHHDisability
			, (select max(case when dv.DVStatus = 1 then 1 else 0 end)
				from lsa.tlsa_Enrollment dv
				where dv.HouseholdID = hh.HouseholdID) as sHHFleeingDV
			, case
				...
				else 99 end as sHoHRace
			,case
				when hoh.ethnicity in ('8','9') then 98
				when hoh.ethnicity in ('0','1') then cast (cast (hoh.ethnicity as varchar) as integer)
				else 99 end as sHoHEthnicity
			, (select max(case when parent.RelationshipToHoH = 2 then 1 else 0 end)
				from lsa.tlsa_Enrollment parent
				where parent.HouseholdID = hh.HouseholdID) as sHHParent
		from lsa.tlsa_Exit ex
		inner join lsa.tlsa_HHID hhid on hhid.HouseholdID = ex.QualifyingExitHHID
		inner join v2020.client hoh on cast (hoh.id as varchar)= ex.hohid and hoh.deleted is false and hoh.parent_id is null
		inner join (
     ...
     update lsa.tlsa_Exit
		set HHVet = sHHVet
		  , HHDisability = sHHDisability
		  , HHFleeingDV = sHHFleeingDV
		  , HoHRace =  sHoHRace
		  , HoHEthnicity = sHoHEthnicity
		  , HHParent = sHHParent
		  , Step = '7.4.1'
	  from subquery
	 where hohid = subquery.shohid and cohort = subquery.scohort and QualifyingExitHHID = subquery.sQualifyingExitHHID;
	...
	inner join (select hhid.HouseholdID, case hhid.ExitCohort
		...
		group by hhid.HouseholdID, case hhid.ExitCohort
			...
			, hhid.ExitCohort
		) ages on ages.HouseholdID = ex.QualifyingExitHHID and ages.Cohort = ex.Cohort and ages.HHType = ex.HHType;
	...
	*/
		...
		from lsa.tlsa_Exit ex
		inner join lsa.tlsa_HHID qx on qx.HouseholdID = ex.QualifyingExitHHID
		left outer join lsa.tlsa_HHID prior on prior.HouseholdID = ex.hohid
			...
			and prior.ExitDate between dateadd('d', -730, qx.EntryDate) and qx.ExitDate;
	...
	*/
	...
	with subquery_7_6_1 as (
		select ex.qualifyingexithhid as qualifyingexithhid,
		case
			when dateadd('d', -1, hhid.EntryDate) < '2012-09-30' then '2012-09-30'
			else dateadd('d', -1, hhid.EntryDate) end as LastInactive
		...
		where ex.Stat <> 5
			or (SELECT prior.EnrollmentID
				...
				where prior.HouseholdID = ex.hohid and case ex.Cohort
	...
	update lsa.tlsa_Exit
		set LastInactive =subquery_7_6_1.LastInactive
	, Step = '7.6.1'
	from subquery_7_6_1
	where lsa.tlsa_exit.qualifyingexithhid  = subquery_7_6_1.qualifyingexithhid;
		...
		inner join lsa.tlsa_HHID possible on possible.HoHID = ex.HoHID and case ex.Cohort
		...
		where
		-- ex.LastInactive is null and
		(possible.ProjectType in (2,3,8,13) or possible.TrackingMethod = 0)
		union
		select distinct ex.hohid, ex.HHType, 1
			, bn.dateprovided
			, case when dateadd('d', 6, bn.dateprovided::date) > cd.CohortEnd then cd.CohortEnd
				else dateadd('d', 6, bn.dateprovided::date)  end
			, '7.6.2.b'
		from lsa.tlsa_Exit ex
		inner join lsa.tlsa_HHID hhid on hhid.HouseholdID = ex.QualifyingExitHHID
		inner join lsa.tlsa_CohortDates cd on cd.Cohort = ex.Cohort
		...
		left outer join
			(select ex.hohid, ex.HHType, max(cal.theDate) as inactive
			  ...
			  left outer join
				 lsa.sys_TimePadded stp on stp.hohid = ex.hohid and stp.HHType = ex.HHType
	...
	*/
	...
	inner join (select distinct ex.hohid, ex.HHType, ex.Cohort
			...
			left outer join lsa.tlsa_HHID es on es.ProjectType in (1,8)
				and es.HouseholdID = ex.hohid and es.EntryDate <= qx.ExitDate
				and (es.ExitDate > ex.LastInactive
				        OR (ex.LastInactive = '2012-09-30' and (es.ExitDate > '2012-09-30' or es.ExitDate is NULL))
					)
				and (es.ActiveHHType = ex.HHType)
			left outer join lsa.tlsa_HHID th on th.ProjectType = 2
				and th.HouseholdID = ex.hohid and th.EntryDate <= qx.ExitDate
				and (th.ExitDate > ex.LastInactive
				        or (ex.LastInactive = '2012-09-30' and (th.ExitDate > '2012-09-30' or th.ExitDate is NULL))
				)
				and (th.ActiveHHType = ex.HHType)
			left outer join lsa.tlsa_HHID rrh on rrh.ProjectType = 13
				and rrh.HouseholdID = ex.hohid and rrh.EntryDate <= qx.ExitDate
				and (rrh.ExitDate > ex.LastInactive
				    or (ex.LastInactive = '2012-09-30' and (rrh.ExitDate > '2012-09-30' or rrh.ExitDate is NULL))
				     )
				and (rrh.ActiveHHType = ex.HHType)
			left outer join lsa.tlsa_HHID psh on psh.ProjectType = 3
				and psh.EntryDate <= qx.ExitDate and psh.HouseholdID = ex.hohid
				and (psh.EntryDate > ex.LastInactive
						or (ex.LastInactive = '2012-09-30' and (psh.ExitDate > '2012-09-30' or psh.ExitDate is NULL))
					)
	...
	*/
	update lsa.tlsa_Exit set hhvet =0 where hhvet is null;
update lsa.tlsa_Exit set hhdisability =0 where hhdisability is null;
update lsa.tlsa_Exit set hhfleeingdv =0 where hhfleeingdv is null;
update lsa.tlsa_Exit set hohrace =0 where hohrace is null;
update lsa.tlsa_Exit set hohethnicity =0 where hohethnicity is null;
update lsa.tlsa_Exit set hhparent =0 where hhparent is null;
	...
	select count (distinct hohid || cast(HHType as varchar))
