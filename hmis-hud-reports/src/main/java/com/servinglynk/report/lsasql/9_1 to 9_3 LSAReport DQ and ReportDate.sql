CREATE OR REPLACE FUNCTION lsa.run_lsareport_dq_and_reportdate (IN reportId integer, IN project_group_code varchar)
RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
...
, case when x.exitdate < cd1.CohortStart then null
	when cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date) is null
	or cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)= '1900-1-1'
	or cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)> n.entrydate
	or (cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)= n.entrydate and n.relationshiptohoh = '1')
	or dateadd('y', 105, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= n.entrydate
	or dateadd('y', 105, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= cd1.CohortStart
	or c.dob_data_quality is null
	or c.dob_data_quality not in ('1','2') then 99
when dateadd('y', 18, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= n.entrydate
	or dateadd('y', 18, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= cd1.CohortStart then 1
else 0 end
, case when cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)is null
		or cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)= '1900-1-1'
		or cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)> n.entrydate
		or (cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)= n.entrydate and n.relationshiptohoh = '1')
		or dateadd('y', 105, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= n.entrydate
		or dateadd('y', 105, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= cd3.CohortStart
		or c.dob_data_quality is null
		or c.dob_data_quality not in ('1','2') then 99
when dateadd('y', 18, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= n.entrydate
	or dateadd('y', 18, cast(replace(convert_from(dob_decrypt(c.dob), 'utf-8'),'.','') as date)) <= cd3.CohortStart  then 1
else 0 end
, case when c.ssn_data_quality in ('8','9') then null
when SUBSTRING(convert_from(ssn_decrypt(c.ssn),'UTF-8'),1,3) in ('000','666')
		--or LENGTH(convert_from(dob_decrypt(c.SSN), 'utf-8')) <> 9
		or SUBSTRING(convert_from(ssn_decrypt(c.ssn),'UTF-8'),4,2) = '00'
		or SUBSTRING(convert_from(ssn_decrypt(c.ssn),'UTF-8'),6,4) ='0000'
		or convert_from(ssn_decrypt(c.ssn),'UTF-8') is null
		or convert_from(ssn_decrypt(c.ssn),'UTF-8') = ''
		or convert_from(ssn_decrypt(c.ssn),'UTF-8') like '%[^0-9]%'
		--5/14/2020 changed below from >= '9'  to = '9')
	--	or left(convert_from(dob_decrypt(c.SSN), 'utf-8'),1) = '9'
		or convert_from(ssn_decrypt(c.ssn),'UTF-8') in ('123456789','111111111','222222222','333333333','444444444'
				,'555555555','777777777','888888888')
	then 0 else 1 end
, '9.1'
from v2020.enrollment n
inner join lsa.lsa_Report rpt on rpt.project_group_code  = n.project_group_code
...
inner join v2020.client c on c.id = n.client_id and c.deleted is false and c.parent_id is null and c.project_group_code  = rpt.project_group_code
left outer join v2020.exit x on x.enrollmentid = n.id
and x.deleted is false and x.parent_id is null and  x.project_group_code  = rpt.project_group_code
...
inner join v2020.moveindate mv on hh.id = mv.enrollmentid and mv.deleted is false and mv.parent_id is null and rpt.project_group_code=hh.project_group_code
inner join v2020.project p on p.id = hh.projectid and p.deleted is false and p.parent_id is null and rpt.project_group_code=hh.project_group_code
inner join v2020.enrollment_coc coc on coc.enrollmentid = hh.id
and coc.coc_code= rpt.ReportCoC
and coc.deleted is false and coc.parent_id is null and rpt.project_group_code=hh.project_group_code
...
and n.deleted is false and n.householdid is not null;
...
update lsa.lsa_Report
	...
	,   NoCoC = (select count (distinct n.householdid)
			...
			left outer join v2020.enrollment_coc coc on
				coc.enrollmentid = n.id AND n.deleted is false
			...
			where n.entrydate <= rpt.ReportEnd
			    and (x.ExitDate is null or x.ExitDate >= (select CohortStart from lsa.tlsa_CohortDates where Cohort = 20))
	...
	,	Race1 = (select count(distinct n.PersonalID)
			from lsa.dq_Enrollment n
			inner join v2020.client c on cast (c.id as varchar) = n.PersonalID AND c.deleted is false and c.parent_id is null
			where n.Status1 is not null and (c.race is null
					or c.race in ('8','9','99')) )
	,	Race3 = (select count(distinct n.PersonalID)
			from lsa.dq_Enrollment n
			inner join v2020.client c on cast (c.id as varchar) = n.PersonalID AND c.deleted is false and c.parent_id is null
			where   (c.race is null
					or c.race in ('8','9','99')))
	...
	,	Destination3 = (select count(distinct n.EnrollmentID)
			from lsa.dq_Enrollment n
			inner join v2020.exit x on CAST (x.enrollmentid AS varchar) = n.EnrollmentID AND x.deleted is false and x.parent_id is null
				and x.deleted is false and parent_id is null
	...
	,	NotOneHoH1 = (select count(distinct n.HouseholdID)
			...
			left outer join (select hn.householdid, count(distinct hn.id) as hoh
				...
				where hn.relationshiptohoh = '1' AND hn.deleted = false and hn.parent_id is null and project_group_code ='LS0024'
	...
	,	NotOneHoH3 = (select count(distinct n.HouseholdID)
			...
			left outer join (select hn.householdid, count(distinct hn.id) as hoh
				from v2020.enrollment hn
				where hn.relationshiptohoh = '1' AND hn.deleted = false and hn.parent_id is null and rpt.project_group_code = hn.project_group_code
				group by hn.householdid
			) hoh on hoh.householdid = n.HouseholdID
	...
	,	MoveInDate1 = (select count(distinct n.EnrollmentID)
			from lsa.dq_Enrollment n
			left outer join v2020.exit x on CAST (x.enrollmentid AS varchar) = n.EnrollmentID
				AND x.deleted = false and x.parent_id is null
			where n.Status1 is not null and n.RelationshipToHoH = 1
				and n.ProjectType in (3,13)
