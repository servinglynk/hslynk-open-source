...
public class RoomServiceImpl extends BaseService implements RoomService {
	...
	public RoomModel createRoom(RoomModel room) {
		...
		ShelterEntity shelterEntity = daoFactory.getShelterRepository().findOne(room.getShelter().getId());
		if(shelterEntity == null) throw new ResourceNotFoundException("Shelter "+room.getShelter().getId()+" not found");
		AreaEntity areaEntity =  daoFactory.getAreaRepository().findOne(room.getArea().getId());
	...
	public void updateRoom(RoomModel room) {
		...
		AreaEntity areaEntity =  daoFactory.getAreaRepository().findOne(room.getArea().getId());
		if(areaEntity == null) throw new ResourceNotFoundException("Area "+room.getArea().getId()+" not found");
		RoomEntity entity =  daoFactory.getRoomRepository().findOne(room.getId());
	...
	public void deleteRoom(UUID roomId) {
		RoomEntity entity =  daoFactory.getRoomRepository().findOne(roomId);
