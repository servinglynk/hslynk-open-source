...
@Component
public class ReRunWorker implements IReRunWorker  {
	
	final static Logger logger = Logger.getLogger(ReRunWorker.class);
	...
	public void processWorkerLine() {
		try {
			List<BulkUpload> uploadEntities=  factory.getBulkUploaderWorkerDao().findBulkUploadByStatusAndYear(UploadStatus.INITIAL.getStatus(),new Long(2017));
			if(uploadEntities!=null && uploadEntities.size() >0 ) {
				for(BulkUpload upload : uploadEntities) {
					FileAppender appender = new FileAppender();
					appender.setName("" + upload.getId());
					appender.setFile("logs/rerun-" + upload.getId() + ".log");
			...
			logger.info("========Rerun Bulk Uploader processed ======");
		} catch (Exception e) {
