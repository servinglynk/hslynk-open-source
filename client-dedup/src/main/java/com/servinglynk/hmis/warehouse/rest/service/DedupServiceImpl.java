...
public class DedupServiceImpl implements DedupService{
	...
	private static final String OPENEMPI_HOST = "openempi.service.url";
	...
	public String authenticate(AuthenticationRequest authRequest) {
		...
		String url = env.getProperty(OPENEMPI_HOST)+"rest/api/v1/openempi-ws-rest/security-resource/authenticate";  
	...
	public Person createUser(Person person,String sessionKey) {
		try {
	        ...
	        String url = env.getProperty(OPENEMPI_HOST)+"rest/api/v1/openempi-ws-rest/person-manager-resource/addPerson";
	...
	public Person updateUser(Person person,String sessionKey) {
		...
		headers.setContentType(mediaType);
        headers.set(OPENEMPI_SESSION_KEY_HEADER, sessionKey);
        String url =  env.getProperty(OPENEMPI_HOST)+"rest/api/v1/openempi-ws-rest/person-manager-resource/updatePerson";
       // requestBody ="{ \"person\": { \"familyName\": \"Anderson\",\"givenName\": \"John\"}}";
	...
	public List<Person> findMatchingPersonsByAttributes(Person person,String sessionKey) {
		...
		headers.setContentType(mediaType);
        headers.set(OPENEMPI_SESSION_KEY_HEADER, sessionKey);
        String url = env.getProperty(OPENEMPI_HOST)+"rest/api/v1/openempi-ws-rest/person-query-resource/findPersonsByAttributes";
        HttpEntity<Object> entityHttp = new HttpEntity<Object>(parsePersonObjectToXMLString(person), headers); 
	...
	*/
	...
	public static void main(String args[]) throws Exception {
		...
		DedupServiceImpl dedup = new DedupServiceImpl();
		  Person person = new Person();
	        person.setFamilyName("Scale");
	        person.setGivenName("Turtle");
	     //  person.setDateOfBirth(formatter.parse("1987-05-25"));
		Person dedupingLogic = dedup.dedupingLogic(person, "430E414FFFAEFF150E12F772D66DCDBA");
