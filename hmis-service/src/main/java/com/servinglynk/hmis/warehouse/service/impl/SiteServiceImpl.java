...
public class SiteServiceImpl extends ServiceBase implements SiteService  {
   ...
   public Site createSite(Site site,UUID projectCocId,String caller){
       com.servinglynk.hmis.warehouse.model.live.Site pSite = SiteConverter.modelToEntity(site, null);
   ...
   public Site updateSite(Site site,UUID projectCocId,String caller){
	   com.servinglynk.hmis.warehouse.model.live.Projectcoc pProjectcoc = daoFactory.getProjectcocDao().getProjectcocById(projectCocId);
       if(pProjectcoc == null) throw new ProjectcocNotFoundException(); 
       com.servinglynk.hmis.warehouse.model.live.Site pSite = daoFactory.getSiteDao().getSiteById(site.getSiteId());
   ...
   public Site deleteSite(UUID siteId,String caller){
       com.servinglynk.hmis.warehouse.model.live.Site pSite = daoFactory.getSiteDao().getSiteById(siteId);
       if(pSite==null) throw new SiteNotFoundException();
   ...
   public Sites getAllProjectCocSites(UUID projectCocId,Integer startIndex, Integer maxItems){
       Sites sites = new Sites();
        List<com.servinglynk.hmis.warehouse.model.live.Site> entities = daoFactory.getSiteDao().getAllProjectCOCSites(projectCocId,startIndex,maxItems);
        for(com.servinglynk.hmis.warehouse.model.live.Site entity : entities){
