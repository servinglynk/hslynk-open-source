...
public class EnrollmentServiceImpl extends ServiceBase implements EnrollmentService {
	...
	public ReleaseOfInfo createReleaseOfInfo(ReleaseOfInfo releaseOfInfo, String caller) {
		...
		for (Project project : releaseOfInfo.getProjects()) {
			...
			daoFactory.getEnrollmentDao().createReleaseOfInfo(releaseOfInfoEntity);
		}
	...
	public ReleaseOfInfo deleteReleaseOfInfo(UUID enrollmentId) {
		...
		List<ReleaseOfInfoEntity> entities = daoFactory.getEnrollmentDao().getreleaseOfInfoEntityByEnrollment(enrollmentId);

		for(ReleaseOfInfoEntity entity : entities)
		{
		daoFactory.getEnrollmentDao().deleteReleaseOfInfo(entity);
		}
		return new ReleaseOfInfo();
	}
