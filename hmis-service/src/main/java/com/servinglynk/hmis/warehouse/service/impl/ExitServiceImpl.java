...
public class ExitServiceImpl extends ServiceBase implements ExitService  {
   ...
   public Exit createExit(Exit exit,UUID enrollmentId,String caller){
       com.servinglynk.hmis.warehouse.model.live.Exit pExit = ExitConverter.modelToEntity(exit, null);
   ...
   public Exit updateExit(Exit exit,UUID enrollmentId,String caller){
	   ...
	   com.servinglynk.hmis.warehouse.model.live.Exit pExit = daoFactory.getExitDao().getExitById(exit.getExitId());
  ...
  //     pExit.setUser(daoFactory.getHmisUserDao().findByUsername(caller));
   ...
   public Exits getAllEnrollmentExits(UUID enrollmentId,Integer startIndex, Integer maxItems){
       Exits exits = new Exits();
        List<com.servinglynk.hmis.warehouse.model.live.Exit> entities = daoFactory.getExitDao().getAllEnrollmentExits(enrollmentId,startIndex,maxItems);
        for(com.servinglynk.hmis.warehouse.model.live.Exit entity : entities){
