...
public class ReportWorker implements IReportWorker  {
	...
	@Transactional
	@Scheduled(initialDelay=20,fixedDelay=10000)
	public void processExportWorkerLine() throws Exception{
					try{
						 FileExports fileExports = serviceFactory.getFileExportService().getFileExportByStatusEmailSent("COMPLETED",false);
						if(fileExports != null) {
							List<FileExport> exports = fileExports.getFileExports();
							if(CollectionUtils.isNotEmpty(exports)) {
								for(FileExport export :  exports) {
									if(export != null) {
										Notification notification = new Notification();
										notification.setMethod("EMAIL");
										notification.setType("HMIS_EMAIL_NOTIFICATION");
										notification.getParameters().addParameter(new Parameter("subject", "Hslynk: Your Apr Report"+export.getName() +" is ready!"));
										StringBuilder builder = new StringBuilder();
										builder.append(" Dear Customer, ");
										builder.append(" <br/> ");
										builder.append(" Your File Export  \""+export.getName() +"\" is ready! ");
										builder.append(" <br/> ");
										builder.append(" Login to the HsLynk's admin application to download your file. ");
										builder.append(" <br/> ");
										builder.append(" https://admin.hslynk.com/ ");
										builder.append(" <br/> ");
										builder.append(" <br/> ");
									...
									//	builder.append("Dear Customer, Your Apr Report \"CHH_Permanent Housing\" is ready!\nLogin to the HsLynk's admin application to download your report.\nhttps://admin.hmislynk.com/\n Thanks \n Hslynk Team ");
										notification.getParameters().addParameter(new Parameter("message", builder.toString()));
										notification.setSender("support@hslynk.com");
										notification.getRecipients().addToRecipient(export.getCreatedBy());
										notificationServiceClient.createNotification(notification);	
										FileExport fileExp = serviceFactory.getFileExportService().getFileExportById(export.getId());
										fileExp.setEmailSent(true);
										serviceFactory.getFileExportService().updateFileExport(export.getId(), fileExp, export.getUsername());
									}
							   }
						   }
						}
